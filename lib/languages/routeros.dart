...
final routeros = Mode(
    refs: {
      ...
      '~contains~2': Mode(className: "string", begin: "\"", end: "\"", contains: [
        ...
        Mode(className: "variable", begin: "\\\$\\(", end: "\\)", contains: [backSlashEscape])
      ]),
    },
    aliases: ["routeros", "mikrotik"],
    caseInsensitive: true,
    lexemes: ":?[\\w-]+",
    keywords: {"literal": "true false yes no nothing nil null", "keyword": "foreach do while for if from to step else on-error and or not in :foreach :do :while :for :if :from :to :step :else :on-error :and :or :not :in :global :local :beep :delay :put :len :typeof :pick :log :time :set :find :environment :terminal :error :execute :parse :resolve :toarray :tobool :toid :toip :toip6 :tonum :tostr :totime"},
    contains: [
      Mode(variants: [Mode(begin: "^@", end: "\$"), Mode(begin: "\\/\\*", end: "\\*\\/"), Mode(begin: "%%", end: "\$"), Mode(begin: "^'", end: "\$"), Mode(begin: "^\\s*\\/[\\w-]+=", end: "\$"), Mode(begin: "\\/\\/", end: "\$"), Mode(begin: "^\\[\\<", end: "\\>\\]\$"), Mode(begin: "<\\/", end: ">"), Mode(begin: "^facet ", end: "\\}"), Mode(begin: "^1\\.\\.(\\d+)\$", end: "\$")], illegal: "."),
      ...
      Mode(begin: "[\\w-]+\\=([^\\s\\{\\}\\[\\]\\(\\)]+)", relevance: 0, returnBegin: true, contains: [
        ...
        Mode(begin: "=", endsWithParent: true, relevance: 0, contains: [Mode(ref: '~contains~2'), Mode(ref: '~contains~3'), Mode(ref: '~contains~2~contains~1'), Mode(className: "literal", begin: "\\b(true|false|yes|no|nothing|nil|null)\\b"), Mode(begin: "(\"[^\"]*\"|[^\\s\\{\\}\\[\\]]+)")])
      ]),
