import 'package:code_highlight/languages/plaintext.dart';
...
class Highlight {
  ...
  List<Mode> _expandMode(Mode mode) {
    ...
    return mode.cachedVariants ?? (mode.endsWithParent == true ? [Mode.inherit(mode)] : [mode]);
  }
  ...
  void _compileMode(Mode mode, [Mode? parent]) {
    ...
    if (parent != null) {
      ...
      if (mode.endsWithParent == true && parent.terminatorEnd != null) {
        mode.terminatorEnd = mode.terminatorEnd! + (mode.end != null ? '|' : '') + parent.terminatorEnd!;
      }
    }
  ...
  Result _parse(
    ...
    dynamic _keywordMatch(Mode mode, RegExpMatch match) {
      var matchStr = langMode.caseInsensitive == true ? match[0]!.toLowerCase() : match[0];
    ...
    List<Node> _processKeywords() {
      ...
      while (match != null) {
        ...
        if (keywordMatch != null) {
          ...
          _addNodes(_buildSpan(keywordMatch[0], [Node(value: match[0])])!, result);
        } else {
          _addText(match[0], result);
        }
        lastIndex = match.start + match[0]!.length;
        match = top!.lexemesRe!.allMatches(modeBuffer, lastIndex).firstWhereOrNull((m) => true);
      }
    ...
    List<Node>? _processSubLanguage() {
      ...
      var result = explicit ? _parse(modeBuffer, language: top!.subLanguage!.first, ignoreIllegals: true, continuation: continuations[top!.subLanguage!.first]) : _parseAuto(modeBuffer, languageSubset: top!.subLanguage!.isNotEmpty ? top!.subLanguage : null);
